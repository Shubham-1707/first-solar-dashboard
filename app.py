...
with tab_forecast:
    ...
    FORECAST_DAYS = 15
    TRAIN_COLOR   = {"RO1": "#0B3D91", "RO2": "#EF6C00"}
    SEV_STYLE     = {
    ...
    if summary_rows:
        st.dataframe(
            pd.DataFrame(summary_rows).set_index(["Train", "KPI"]),
            use_container_width=True)
    st.markdown("")
 
    # ── 4-panel forecast chart (both trains, 15-day window) ───────
    fig = make_subplots(rows=2, cols=2,
                        subplot_titles=[c[1] for c in KPI_CONFIGS],
                        vertical_spacing=0.14, horizontal_spacing=0.08)
    legend_trains, legend_sev = set(), set()
    ...
    for idx, (col, ylabel, sign, thresholds) in enumerate(KPI_CONFIGS):
        ...
        for train in train_pick:
            ...
            # Historical line
            fig.add_trace(go.Scatter(
                x=x, y=y, mode="lines", name=train,
                line=dict(color=clr, width=2),
                legendgroup=train, showlegend=(train not in legend_trains)
            ), row=r, col=c)
            ...
            yf = m * xf + b
            fig.add_trace(go.Scatter(
                x=future, y=yf, mode="lines", name=f"{train} (forecast)",
                line=dict(color=clr, width=1.5, dash="dot"),
                legendgroup=f"{train}_fc", showlegend=(train not in legend_trains)
            ), row=r, col=c)
 
        # Threshold lines for all three severities
        for sev, target in thresholds.items():
            st_s = SEV_STYLE[sev]
            fig.add_hline(
                y=target,
